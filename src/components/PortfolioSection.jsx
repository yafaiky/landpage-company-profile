...
export default function PortfolioSection() {
  ...
  useEffect(() => {
    if (gridRef.current.length > 0) {
      gsap.fromTo(
        gridRef.current.filter(Boolean),
  ...
  return (
    <>
      <section ref={sectionRef} className="py-20 md:py-32 min-h-screen">
            ...
            {categories.map((cat, i) => (
              <button
                ...
                className={`relative px-6 py-3 rounded-full text-xs md:text-sm font-bold tracking-widest uppercase transition-all duration-300 overflow-hidden group border ${
                  activeTab === cat
                    ? "border-black text-white bg-black shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
                    : "border-black/30 text-black hover:border-black hover:text-white hover:bg-black"
                }`}
              >
                <div
                  className={`absolute inset-0 bg-black/20 transition-transform duration-300 ease-out z-0 ${
                    activeTab === cat ? "translate-y-0" : "translate-y-[101%]"
                  }`}
                />
          ...
          >
            ...
            >
              ...
              <div className="p-8 md:p-12 shrink-0">
                ...
                <p className="text-zinc-600 mb-8">{selectedItem.description}</p>
