...
export default function TermsSection() {
  ...
  return (
    <section
      ref={sectionRef}
      id="terms"
      className="relative overflow-hidden bg-zinc-50 py-24 md:py-36"
    >
      {/* Ambient blob */}
      <div
      ...
      <div className="container relative z-10">
          ...
          {terms.map((term) => {
            ...
            return (
              <div
                ...
                className="relative rounded-2xl border cursor-pointer overflow-hidden group shadow-sm"
                style={{
                  background: isOpen
                    ? `linear-gradient(145deg, ${term.color}0d 0%, #ffffff 60%)`
                    : '#ffffff',
                  borderColor: isOpen
                    ? term.color + '40'
                    : isHovered
                  ...
                  boxShadow: isOpen
                    ? `0 15px 40px rgba(0,0,0,0.1), 0 0 0 1px ${term.color}15`
                    : isHovered
              ...
              >
                ...
                <div
                  ...
                  style={{
                    color: isOpen ? term.color : 'rgba(0,0,0,0.08)',
                  }}
                >
                ...
                <div className="p-7">
                  ...
                  <div
                    ...
                    style={{
                      background: isOpen ? term.color + '15' : 'rgba(0,0,0,0.02)',
                      color: isOpen ? term.color : 'rgba(0,0,0,0.3)',
                      border: `1px solid ${isOpen ? term.color + '30' : 'rgba(0,0,0,0.04)'}`,
                  ...
                  {/* Title & Summary */}
                  <h3 className="text-black font-bold text-lg mb-2 leading-tight transition-colors duration-300">
                    {term.title}
                  </h3>
                  <p className="text-zinc-600 text-sm leading-relaxed mb-4">{term.summary}</p>

                  {/* See more / less */}
                  <div
                    className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em] transition-all duration-300"
                    style={{ color: isOpen ? term.color : 'rgba(0,0,0,0.3)' }}
                  >
                    ...
                    >
                      <ul className="flex flex-col gap-3">
                        {term.details.map((d, idx) => (
                          <li
                            key={idx}
                            className="flex items-start gap-3 text-[0.82rem] text-zinc-600 leading-relaxed"
                          >
                            <span
        ...
        {/* Bottom CTA strip */}
        <div className="mt-16 md:mt-20 p-8 md:p-10 rounded-2xl border border-black/5 bg-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm">
          <div>
            <p className="text-black font-bold text-xl md:text-2xl mb-2">
              lorem ipsum dolor sit amet
            </p>
            <p className="text-zinc-500 text-sm max-w-[50ch]">
